...
const Sidebar = ({ isCollapsed, onToggleCollapse, mobileOpen, onMobileClose }) => {
  ...
  const SidebarContent = ({ collapsed }) => (
    <div className="flex flex-col h-full">
      ...
      {/* ── HEADER ── */}
      <div className="flex items-center h-16 px-4 border-b border-slate-800 overflow-hidden flex-shrink-0">
        ...
        {onMobileClose && (
          <button
            ...
            className="ml-auto flex-shrink-0 flex h-7 w-7 items-center justify-center rounded-lg text-slate-400 hover:text-white hover:bg-slate-800 transition-all"
          >
            <X size={15} />
          </button>
        )}
              ...
              {items.map(item => {
                ...
                return (
                  <NavLink
                    ...
                    onClick={onMobileClose}   // close drawer on nav
                    className={({ isActive }) =>
      ...
      {/* ── FOOTER (user + logout) ── */}
      <div className="border-t border-slate-800 bg-slate-950/50 flex-shrink-0">
        {!collapsed && user && (
  ...
  return (
    <>
      {/* ════════════════════════════════════
          ...
          ════════════════════════════════════ */}
      <div
        className={`hidden md:flex fixed top-0 left-0 h-screen flex-col bg-slate-900 text-slate-300 border-r border-slate-800 transition-all duration-300 z-50 ${
      ...
      <div
        className={`fixed top-0 left-0 h-full w-72 bg-slate-900 text-slate-300 border-r border-slate-800 z-50 flex flex-col transition-transform duration-300 md:hidden ${
